...
export class AuthService {
    constructor(private userService: UserService, private jwtService: JwtService, private instructService: InstructorService, private adminService: AdminService){}
    ...
    public async getInstructorCreds(email: string, password: string){
        ...
        if(!(await argon2.verify(password, instructor.password))) throw new HttpException("Email or Password Incorrect", HttpStatus.UNAUTHORIZED);
        return instructor;
    }
    ...
    public async getAdminCreds(email: string, password: string){
        ...
        if(!(await argon2.verify(password, admin.password))) throw new HttpException("Email or Password Incorrect", HttpStatus.UNAUTHORIZED);
        return admin;
    }
}
